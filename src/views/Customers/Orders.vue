    <template>
        ...
        <div class="container mx-auto p-4 max-w-4xl">
            ...
            <div class="flex items-center flex-col bg-white p-6 rounded-lg shadow-lg mb-4 w-full">
                ...
                <p class="text-lg text-left md:text-center font-bold mb-2">
                    {{ question.text }}
        ...
        export default {
            ...
            const fetchQuestions = async () => {
                ...
                if (question.type === 'text') {
                    return { type: 'text', value: '', required: question.required };
                } else if (question.type === 'checkbox') {
                    return { type: 'checkbox', value: [], required: question.required };
                } else if (question.type === 'radio') {
                    return { type: 'radio', value: '', required: question.required };
                } else if (question.type === 'star') {
                    return { type: 'star', value: 0, required: question.required };
                } else if (question.type === 'file') {
                    return { type: 'file', value: null, required: question.required };
                } else if (question.type === 'date') {
                    return { type: 'date', value: '', required: question.required };
                } else if (question.type === 'time') {
                    return { type: 'time', value: '', required: question.required };
                }
                return { type: 'text', value: '', required: question.required };
                });
            };
            ...
            const updateCheckboxResponse = (index, label) => {
                const currentValues = responses.value[index].value;
                const newValues = currentValues.includes(label)
                ? currentValues.filter(value => value !== label)
                : [...currentValues, label];
                responses.value[index].value = newValues;
            };
            ...
            const submitSurvey = async () => {
                const incomplete = questions.value.some((question, index) => {
                if (question.required) {
                    const response = responses.value[index];
                    if (question.type === 'checkbox' && response.value.length === 0) {
                    return true;
                    }
